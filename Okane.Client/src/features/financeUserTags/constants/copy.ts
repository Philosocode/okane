export const FINANCE_USER_TAGS_COPY = {
  COMBOBOX: {
    ERRORS: {
      CREATE: 'Error creating tag. Refresh the page and try again.',
      FETCH: 'Error fetching tags. Refresh the page and try again.',
    },
  },

  DELETE_MODAL: {
    CONFIRMATION: 'You are about to delete the following finance tag:',
    ERROR: 'Error deleting finance tag. Please refresh the page and try again.',
    HEADING: 'Delete Finance Tag',
  },

  MANAGE_PAGE: {
    DELETE_FINANCE_TAG: 'Delete finance tag.',
    HEADING: 'Manage Finance Tags',
    FETCH_TAGS_ERROR: 'Error fetching finance user tags. Refresh the page and try again.',
    NAME: 'Name',
    NO_FINANCE_TAGS_TO_DISPLAY: `No finance tags to display. To create a tag, click the bottom in the bottom-right corner of the screen.`,
    RENAME_FINANCE_TAG: 'Rename finance tag.',
    TAG_TYPE: 'Tag Type',
    TYPE: 'Type',
  },

  RENAME_MODAL: {
    ERROR: 'Error renaming finance tag. Please refresh the page and try again.',
    HEADING: 'Rename Finance Tag',
    UPDATED_NAME: 'Updated name',
  },
}
